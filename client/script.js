 ...
 */
var chargeCard = function (paymentMethod, customer) {
  fetch('/charge-card-off-session', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ paymentMethod: paymentMethod, customer }),
  })
...
function chargeSavedCard() {
  showEl('.saved-card')
  showEl('.saved-card-choose')
  hideEl('.sr-select-pm')
}

document.querySelector('#fetch-saved-cards').addEventListener('click', (e) => {
  changeLoadingState(true, '#fetch-saved-cards')
  fetch(
    ...
    .then((cards) => {
      cards.forEach((card) => {
        document.querySelector(
          '#saved-cards-select'
        ).innerHTML += `<option value="${card.id}">
          ${card.brand} ****${card.last4} exp ${card.exp}
        </option>`
      })
      changeLoadingState(false, '#fetch-saved-cards')
    })
    .catch((err) => {
      console.log(err)``
      changeLoadingState(false, '#fetch-saved-cards')
...
document.querySelector('#saved-card-pay').addEventListener('click', (e) => {
  e.preventDefault()
  var selectedSavedCard = document.querySelector('#saved-cards-select option:checked').value
  chargeCard(selectedSavedCard, document.querySelector('#customer-id').value)
})
...
document.querySelector('form').addEventListener('submit', function (evt) {
  ...
  if (selectedPaymentMethod === 'pm_card_chooseSaved') {
    chargeSavedCard()
  } else {
